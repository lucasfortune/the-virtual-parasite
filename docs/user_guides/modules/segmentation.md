---
title: "Segmentation Module Guide"
module: segmentation
version: "1.0.0"
---
...
![Segmentation Module Overview](screenshots/segmentation-overview.png)
...
For more background, see [Segmentation Overview](info:segmentation).
...
This step offers two mutually exclusive workflows. Expanding one section will collapse the other. For help choosing, see [Workflow Choice](info:segmentation.step1.workflow-choice).

![Step 1 Workflow Selection](screenshots/segmentation-step1.png)
...
|-------|--------|-------------|
| **Raw Images** | TIFF stack | Your microscopy images for training. See [Raw Images](info:segmentation.step1.raw-images) |
| **Annotations** | TIFF stack | Corresponding segmentation masks (same dimensions). See [Annotations](info:segmentation.step1.annotations) |
...
| Input | Format | Description |
|-------|--------|-------------|
| **Model File** | .pth | PyTorch model weights. See [Model File](info:segmentation.step1.model-file) |
| **Config File** | .json | Model configuration (architecture, classes). See [Config File](info:segmentation.step1.config-file) |
...
![Step 2 Configuration](screenshots/segmentation-step2.png)
...
|-----------|---------|---------|--------------|
| **Patch Size** | 64 | 32, 48, 64, 96, 128, 256 | [Patch Size](info:segmentation.config.patch-size) |
| **Patches per Image** | 50 | 1–100 | [Patches per Image](info:segmentation.config.patches-per-image) |
| **Batch Size** | 8 | 1, 2, 4, 8, 16, 32 | [Batch Size](info:segmentation.config.batch-size) |
| **Data Augmentation** | Enabled | On/Off | [Augmentation](info:segmentation.config.augmentation) |

#### Model Architecture

| Parameter | Default | Options | Help Article |
|-----------|---------|---------|--------------|
| **Number of Features** | 64 | 32, 48, 64, 96, 128 | [Number of Features](info:segmentation.config.num-features) |
| **Number of Layers** | 4 | 2–6 | [Number of Layers](info:segmentation.config.num-layers) |

#### Training Parameters

| Parameter | Default | Options | Help Article |
|-----------|---------|---------|--------------|
| **Learning Rate** | 1e-3 | 1e-5 to 2e-3 | [Learning Rate](info:segmentation.config.learning-rate) |
| **Number of Epochs** | 100 | 10–500 | [Number of Epochs](info:segmentation.config.num-epochs) |
...
Monitor your model as it learns to segment your images. For details on interpreting training metrics, see [Training Progress](info:segmentation.step3).

![Step 3 Training](screenshots/segmentation-step3.png)
...
|-------|-------------|--------------|
| **Loss Curves** | Shows training and validation loss over epochs | [Loss Curves](info:segmentation.step3.loss-curves) |
| **Dice Score** | Shows segmentation accuracy over epochs | [Dice Score](info:segmentation.step3.dice-score) |
...
Apply your trained (or imported) model to segment new data. See [Inference Overview](info:segmentation.step4).

![Step 4 Inference](screenshots/segmentation-step4.png)
...
For data requirements, see [Inference Data](info:segmentation.step4.inference-data).
...
| Action | Description |
|--------|-------------|
| **Open in Image Viewer** | View your segmentation results immediately. See the [Image Viewer Guide](imageviewer.md) for navigation controls |
...
### Module Overview
- [Segmentation Overview](info:segmentation)

### Step 1: Data Selection
- [Data Upload](info:segmentation.step1)
- [Workflow Choice](info:segmentation.step1.workflow-choice)
- [Raw Images](info:segmentation.step1.raw-images)
- [Annotations](info:segmentation.step1.annotations)
- [Model File](info:segmentation.step1.model-file)
- [Config File](info:segmentation.step1.config-file)

### Step 2: Configuration
- [Patch Size](info:segmentation.config.patch-size)
- [Patches per Image](info:segmentation.config.patches-per-image)
- [Batch Size (Segmentation)](info:segmentation.config.batch-size)
- [Augmentation](info:segmentation.config.augmentation)
- [Number of Features](info:segmentation.config.num-features)
- [Number of Layers](info:segmentation.config.num-layers)
- [Learning Rate (Segmentation)](info:segmentation.config.learning-rate)
- [Number of Epochs](info:segmentation.config.num-epochs)

### Step 3: Training
- [Training Progress](info:segmentation.step3)
- [Loss Curves](info:segmentation.step3.loss-curves)
- [Dice Score](info:segmentation.step3.dice-score)

### Step 4: Inference
- [Run Inference](info:segmentation.step4)
- [Inference Data](info:segmentation.step4.inference-data)

---

## Next Steps

After segmenting your images, you can:
- **View results** — Use the Image Viewer to inspect the segmentation result
- **Generate a surface mesh** — Use the surface mesh generation module to turn your segmentation into a 3dimensional model
- **View model in 3D** — Open the generated mesh in the 3D visualization module
