...
| Format | Description |
|--------|-------------|
| **VoxelSlices** | The default output of the Mesh Generation module — voxel data slice by slice; enables the per-class slice-range controls |
...
A summary card confirms the number of classes and the detected format after selection.
...
| Action | How | Effect |
|--------|-----|--------|
| **Rotate** | Left-click + drag (or right-click + drag) | Rotates mesh around X/Y axes |
...
| Control | Purpose |
|---------|---------|
| **Visibility** | Toggle original data planes on/off (starts hidden) |
| **Opacity** | Adjust transparency (5% – 100%, default 30%) |
| **Slice Range** | Show specific depth regions (appears only when the source has more than one slice) |
...
- Collapse the controls panel with its **×** button and bring it back with the **Controls** button
- Press **Esc** to exit

> **Notes:** When the mesh was generated with a non-cubic Z voxel scale, that ratio is stored in the mesh metadata and applied automatically. Your control settings — class visibility, opacity, slice ranges, and camera position — are remembered when you leave the viewer and return during the same session.
